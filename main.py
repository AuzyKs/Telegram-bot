...
token = "5516968944:AAEzI8r0gb4mQqeeE6d7FX0jcYX-2402PVg"  # Токен телеграм бота
admin_id = 674462998  # ID админа
admin_link = "@FARUMs"  # Ссылка на админа с @ в начале
link = "@MiliyslivBot"  # Ссылка на бота без @ в начале
    ...
    if not (db.get_users_exist(message.chat.id)):
        if (message.text != "💼 Профиль" and message.text.startswith("/start ")):
            ...
            if (int(message.chat.id) != int(_ref)):
                db.add_user_to_db(
                    message.chat.id, message.chat.username, _ref, db.get_settings()[4])
                db.set_balance(_ref, db.get_balance(
                    _ref) + db.get_settings()[5])
                ...
                db.add_user_to_db(
                    message.chat.id, message.chat.username, 0, db.get_settings()[4])
                await bot.send_message(chat_id=admin_id, text=f"Новый пользователь: {_user_id} (@{_username})")
        else:
            db.add_user_to_db(
                message.chat.id, message.chat.username, 0, db.get_settings()[4])
